...
#define LEDPIN 4 
#define LEDCOUNT 25
#define analogPinForRV 1 // change to pins to the analog pins used
...
int led = 13;
int timeLagOn = 2000;
unsigned long lastOnMillis;
int windSpeedThresh = 6;
...
int sensorReading;      // the analog reading from the windspeed sensor
...
void loop() {
  ...
  if (millis() - lastMillis > 200){ // read every 200 ms - printing slows this down further
    ...
    if (WindSpeed_MPH >= windSpeedThresh){
      ...
      if (millis() - lastOnMillis > timeLagOn){
  ...
  sensorReading = WindSpeed_MPH;
  Serial.print(" SensorReading::"); Serial.println((float)sensorReading);
  
  uint16_t constLevel = constrain(sensorReading, 0, 16);
  Serial.print(" constLevel::"); Serial.println((float)constLevel);

//  uint16_t scaledLevel = map(constLevel,0,20, 1,LEDCOUNT-1);
//  Serial.print(" scaledLevel::"); Serial.println((float)scaledLevel);


  // colorLevel(strip.Color(255, 0, 0), sensorReading); // Red
  colorLevel(strip.Color(0, 255, 0), constLevel); // Green
...
void colorLevel(uint32_t c, int level) {
  ...
  // uint16_t scaledLevel = map(level,0,700,1,LEDCOUNT-1);  // fsr
  uint16_t scaledLevel = map(level,0,16, 1,LEDCOUNT-1);  // for alcohol sensor
